...
public class ScriptEditorTool implements AiToolPlugin {
	...
	@Tool(value = {
		"Check if the script editor is open.",
	...
	@Tool(value = {
		"Start the script editor if it's not currently open",
	...
	public String writeScript(@P("id") final String id, @P("content") final String content) {
		try {
			// Validate inputs
			...
			if (scriptID.editorIndex < 0 || scriptID.editorIndex >= TextEditor.instances.size()) {
			...
			SwingUtilities.invokeAndWait(() -> {
				try {
					...
					result[0] = "ERROR: Failed to update tab at index " + scriptID.tabIndex + ": " + e.getMessage();
				}
			});
			return result[0];
		} catch (Exception e) {
	...
	public String setScriptFilename(@P("id") final String id, @P("filename") final String filename) {
		try {
			// Validate inputs
			if (id == null || id.trim().isEmpty()) {
				return "ERROR: Script id cannot be null or empty";
	...
	private String getTabJson(TextEditorTab tab, int editorIndex, int tabIndex) {
			...
			sb.append("  \"id\": \"").append(editorIndex).append(":").append(tabIndex).append("\"\n");
			sb.append("}\n");
			return sb.toString();
	}
	...
	private void performRenameScript(final TextEditor textEditor, final ScriptID scriptID, final String name, final String[] result) {
		try {
			// Validate tab index
			final TextEditorTab tab = textEditor.getTab(scriptID.tabIndex);
			...
			result[0] = "ERROR: Failed to rename tab at index " + scriptID.tabIndex + ": " + e.getMessage();
		}
	}
