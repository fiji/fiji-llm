...
public class DefaultConversationService extends AbstractService implements ConversationService {
    ...
    public List<String> getConversationNames() {
        return conversations.stream().map(Conversation::name).collect(Collectors.toList());
    }
    ...
    @Override
    public Conversation createConversation(String name, SystemMessage systemMessage) {
    ...
    public void initialize() {
        ...
        if (!conversationDir.exists()) {
            if (!conversationDir.mkdirs()) {
                ...
                conversationDir = new File(System.getProperty("user.home"), ".fiji-chat-history");
                if (!conversationDir.exists()) {
    ...
    private void loadConversations() {
        ...
        File[] files = conversationDir.listFiles((dir, name) -> name.endsWith(".json"));
        if (files != null) {
            // Sort by last modified date, most recent first
            Arrays.sort(files, (a, b) -> Long.compare(b.lastModified(), a.lastModified()));
            for (File file : files) {
                try {
                    SerializedConversation serialized = gson.fromJson(
                        new FileReader(file),
                        SerializedConversation.class
                    );

                    if (serialized != null) {
                        SystemMessage systemMessage = new SystemMessage(serialized.getSystemMessage());
                        Conversation conversation = new Conversation(serialized.getName(), systemMessage);

                        for (SerializedConversation.SerializedConversationMessage msg : serialized.getMessages()) {
                            ChatMessage memoryMessage = ChatMessageConverter.fromSerialized(msg.getMemoryMessage());
                            conversation.addMessage(msg.getDisplayMessage(), memoryMessage);
                        }

                        conversationsByName.put(conversation.name(), conversation);
                        conversations.add(conversation);
                        conversationLengths.put(conversation.name(), conversation.messages().size());
                    }
                } catch (IOException e) {
    ...
    private void saveConversations() {
        for (Conversation conversation : conversations) {
            ...
            int previousLength = conversationLengths.getOrDefault(conversation.name(), -1);
    ...
    private void saveConversation(Conversation conversation) {
        try {
            File file = new File(conversationDir, sanitizeFileName(conversation.name()) + ".json");
            ...
            List<SerializedConversation.SerializedConversationMessage> messages = new ArrayList<>();
            for (Conversation.Message msg : conversation.messages()) {
                ...
                serializedMsg.setMemoryMessage(ChatMessageConverter.toSerialized(msg.memory()));
