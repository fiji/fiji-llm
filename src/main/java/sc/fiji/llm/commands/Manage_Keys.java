...
/**
 * Manage API keys for different AI providers. Provides a user interface to add,
 * update, remove, and validate API keys.
 */
@Plugin(type = Command.class, description = "Manage API keys for AI providers",
	iconPath = "/icons/robot-icon-32.png", menu = { @Menu(label = "Help"), @Menu(
		label = "Assistants"), @Menu(label = "Manage API Keys...") })
public class Manage_Keys extends DynamicCommand {
	...
	@Parameter(label = "Get API Key →", visibility = ItemVisibility.MESSAGE,
		description = "Where to get an API key for this provider", persist = false,
		required = false)
	private String apiKeyLink = "";

	@Parameter(label = "Enter API Key →",
		description = "New API key for this provider", persist = false)
	...
	public void initialize() {
		...
		final List<String> providerNames = providers.stream().filter(
			LLMProvider::requiresApiKey).map(LLMProvider::getName).collect(Collectors
				.toList());
		...
		if (!singleProvider) {
			...
			providerMsg.append("<div style='width: " + WIDTH +
		...
		apiKeyMsg.append(singleProvider ? "<div style='width: " + WIDTH +
			"px;'><hr style='border: none; border-top: 2px solid #cccccc; margin: 0;'></div>"
			: "");
		...
		if (singleProvider && prefService.getBoolean(Manage_Keys.class, autoRunKey(
			provider), false))
		{
	...
	/**
	 * Callback triggered when the provider selection changes. Updates the API key
	 * link and shows existing key status.
	 */
	protected void providerChanged() {
		...
		final MutableModuleItem<String> apiKeyLinkItem = getInfo().getMutableInput(
			"apiKeyLink", String.class);
		apiKeyLinkItem.setValue(this, "<a href=\"" + apiKeyUrl + "\">" + apiKeyUrl +
			"</a>");

		// Check if we have a stored API key for this provider
		final String storedKey = apiKeyService.getApiKey(provider);
		final MutableModuleItem<String> apiKeyItem = getInfo().getMutableInput(
			"apiKey", String.class);

		if (storedKey != null && !storedKey.isEmpty()) {
			apiKeyItem.setValue(this, MASK); // Mask the stored key
			apiKeyItem.setDescription(
				"Key is configured (enter new key to replace it)");
		}
		else {
			apiKeyItem.setValue(this, "");
			apiKeyItem.setDescription("Enter your " + provider +
				" API key from the link above");
		}
	}
	...
	public void run() {
		...
		if (apiKeyService.hasApiKey(provider)) {
			...
			if (startChatbot) {
				...
				String model = prefService.get(Fiji_Chat.class,
					Fiji_Chat.LAST_CHAT_MODEL);
				try {
	...
	/**
	 * @param provider
	 * @return A key to use with the {@link PrefService} for this particular
	 *         provider
	 */
